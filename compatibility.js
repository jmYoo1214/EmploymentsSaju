...
class CompatibilityCalculator {
  ...
  extractSajuElements(person) {
    ...
    const hour = this.convertBirthTimeToHour(
      person.birthHour,
      person.birthMinute
    );
  ...
  calculateAccurateGanji(year, month, day, hour) {
    ...
    // 조정된 날짜로 간지 재계산
    const adjustedGanji = this.calculateGanjiFromDate(
      adjustedYear,
      adjustedMonth,
      adjustedDay
    );
  ...
  calculateYinYangBalance(ganji) {
    ...
    const balance =
      ganYinYang[ganji.yearGan] +
      ganYinYang[ganji.monthGan] +
      ganYinYang[ganji.dayGan] +
      ganYinYang[ganji.hourGan] +
      jiYinYang[ganji.yearJi] +
      jiYinYang[ganji.monthJi] +
      jiYinYang[ganji.dayJi] +
      jiYinYang[ganji.hourJi];
  ...
  calculateOverallCompatibility(person1, person2) {
    ...
    const fiveElementScore =
      this.calculations.calculateFiveElementCompatibility(person1, person2);
    score += fiveElementScore * 0.25;

    // 2. 일간(日干) 궁합 (20%)
    const ilganScore = this.calculations.calculateIlganCompatibility(
      person1,
      person2
    );
    score += ilganScore * 0.2;

    // 3. 연지·월지 궁합 (15%)
    const yearMonthScore = this.calculations.calculateYearMonthCompatibility(
      person1,
      person2
    );
    ...
    // 4. 십성 비교 (15%)
    const sipsungScore = this.calculations.calculateSipsungCompatibility(
      person1,
      person2
    );
    ...
    // 5. 음양 밸런스 (10%)
    const yinYangScore = this.calculations.calculateYinYangCompatibility(
      person1,
      person2
    );
    score += yinYangScore * 0.1;

    // 6. 합충형파해 여부 (10%)
    const harmonyScore = this.calculations.calculateHarmonyCompatibility(
      person1,
      person2
    );
    score += harmonyScore * 0.1;

    // 7. 귀인성 (5%)
    const guinScore = this.calculations.calculateGuinCompatibility(
      person1,
      person2
    );
  ...
  calculateIntimacyCompatibility(person1, person2) {
    ...
    // 1. 음양 에너지 조화 (25%)
    const yinYangEnergyScore =
      this.calculations.calculateYinYangEnergyCompatibility(person1, person2);
    score += yinYangEnergyScore * 0.25;

    // 2. 식상·재성·관성의 흐름 (25%)
    const sipsungFlowScore =
      this.calculations.calculateSipsungFlowCompatibility(person1, person2);
    score += sipsungFlowScore * 0.25;

    // 3. 일지(日支) 궁합 (20%)
    const dayJiScore = this.calculations.calculateDayJiCompatibility(
      person1,
      person2
    );
    ...
    // 4. 홍염·천희·월하 등 애정살 (15%)
    const loveStarScore = this.calculations.calculateLoveStarCompatibility(
      person1,
      person2
    );
    ...
    // 5. 생기운 순환 (10%)
    const energyFlowScore = this.calculations.calculateEnergyFlowCompatibility(
      person1,
      person2
    );
    score += energyFlowScore * 0.1;

    // 6. 체질(냉/열, 습/건) (5%)
    const constitutionScore =
      this.calculations.calculateConstitutionCompatibility(person1, person2);
  ...
  calculatePersonalityScore(person1, person2) {
    ...
    // 년지 궁합
    const yearCompatibility = this.getYearCompatibility(
      person1.year,
      person2.year
    );
    ...
    // 월지 궁합
    const monthCompatibility = this.getMonthCompatibility(
      person1.month,
      person2.month
    );
  ...
  calculateFortuneScore(person1, person2) {
    ...
    // 오행 상생 관계
    const fiveElementCompatibility = this.getFiveElementCompatibility(
      person1,
      person2
    );
  ...
  getCompatibilityType(overallScore, intimacyScore) {
    if (overallScore >= 90 && intimacyScore >= 90) {
      return {
        type: "천생연분형",
        emoji: "💖",
        description: "운명적 연결, 안정+열정",
      };
    } else if (overallScore >= 70 && intimacyScore >= 70) {
      return {
        type: "현실적 조화형",
        emoji: "💕",
        description: "다툼 있으나 궁합 좋음",
      };
  ...
  generateDetailedAnalysis(person1, person2, overallScore, intimacyScore) {
    ...
    analysis.push(`<div class="analysis-item">
      <h4>💫 십성 분석</h4>
      <p><strong>재성(애정):</strong> ${this.getSipsungCount(person1.sipsung, [
        "정재",
        "편재",
      ])} vs ${this.getSipsungCount(person2.sipsung, ["정재", "편재"])}</p>
      <p><strong>관성(책임감):</strong> ${this.getSipsungCount(
        person1.sipsung,
        ["정관", "편관"]
      )} vs ${this.getSipsungCount(person2.sipsung, ["정관", "편관"])}</p>
  ...
  generateDetailedAdvice(overallScore, intimacyScore, compatibilityType) {
    ...
    if (overallScore >= 90) {
      advice.push(
        "💖 <strong>천생연분형</strong><br>운명적인 만남입니다. 서로를 완벽하게 이해하고 조화롭게 지낼 수 있는 이상적인 궁합입니다."
      );
    } else if (overallScore >= 80) {
      advice.push(
        "💕 <strong>매우 좋은 궁합</strong><br>서로를 잘 이해하고 조화롭게 지낼 수 있을 것입니다. 소중한 관계를 유지해 나가세요."
      );
    } else if (overallScore >= 70) {
      advice.push(
        "💗 <strong>좋은 궁합</strong><br>서로의 차이점을 인정하고 보완해 나가면 더욱 좋은 관계가 될 것입니다."
      );
    } else if (overallScore >= 50) {
      advice.push(
        "🤝 <strong>보통의 궁합</strong><br>서로의 장점을 살리고 단점을 보완하는 노력이 필요합니다."
      );
    } else {
      advice.push(
        "💪 <strong>노력이 필요한 궁합</strong><br>서로에 대한 이해와 노력으로 좋은 관계를 만들어 갈 수 있습니다."
      );
    }
    ...
    if (intimacyScore >= 90) {
      advice.push(
        "🔥 <strong>강렬한 끌림 + 에너지 완벽조화</strong><br>서로에게 강한 매력을 느끼며 감정적 교감이 뛰어납니다."
      );
    } else if (intimacyScore >= 70) {
      advice.push(
        "❤️ <strong>열정적이며 안정적</strong><br>속도가 잘 맞으며 깊은 정신적 교감이 가능합니다."
      );
    } else if (intimacyScore >= 50) {
      advice.push(
        "💙 <strong>평균적 친밀감</strong><br>한쪽이 주도적인 관계가 될 수 있으니 균형을 맞춰가세요."
      );
    } else {
      advice.push(
        "💔 <strong>감정적 불균형</strong><br>욕구 불균형이나 에너지 단절이 있을 수 있으니 소통이 중요합니다."
      );
    }

    // 궁합 타입별 조언
    advice.push(
      `<br><strong>${compatibilityType.emoji} ${compatibilityType.type}</strong><br>${compatibilityType.description}`
    );
  ...
  validateInputs() {
    const requiredFields = [
      this.person1Name,
      this.person1Gender,
      this.person1Calendar,
      this.person1BirthDate,
      this.person1BirthHour,
      this.person1BirthMinute,
      this.person2Name,
      this.person2Gender,
      this.person2Calendar,
      this.person2BirthDate,
      this.person2BirthHour,
      this.person2BirthMinute,
    ];
  ...
  showLoading() {
    this.calculateButton.innerHTML =
      '<span class="button-icon">⏳</span><span class="button-text">계산 중...</span>';
  ...
  displayResults(person1, person2, compatibility) {
    ...
    // 버튼 복원
    this.calculateButton.innerHTML =
      '<span class="button-icon">💕</span><span class="button-text">궁합 계산하기</span>';
    this.calculateButton.disabled = false;
  }
  ...
  // 현대적 궁합 점수 계산 함수들 (75점 목표 조정)
  calculateModernOverallCompatibility(person1, person2) {
    let score = 60; // 기본 점수를 60으로 상향 조정

    // 1. 오행 상생/상극 분석 (20점) - 가중치 감소
    const fiveElementScore = this.calculateFiveElementCompatibility(
      person1,
      person2
    );
    score += fiveElementScore * 0.2;

    // 2. 일간 궁합 (15점) - 가중치 감소
    const ilganScore = this.calculateIlganCompatibility(person1, person2);
    score += ilganScore * 0.15;
    ...
    // 5. 귀인성 제거 - 너무 높은 점수 방지

    return Math.max(0, Math.min(100, Math.round(score)));
  }

  calculateModernIntimacyCompatibility(person1, person2) {
    let score = 55; // 기본 점수를 55로 상향 조정

    // 1. 음양 에너지 조화 (20점) - 가중치 감소
    const yinYangEnergy = this.calculateYinYangEnergyCompatibility(
      person1,
      person2
    );
    score += yinYangEnergy * 0.2;

    // 2. 십성 흐름 (15점) - 가중치 감소
    const sipsungFlow = this.calculateSipsungFlowCompatibility(
      person1,
      person2
    );
    score += sipsungFlow * 0.15;

    // 3. 일지 궁합 (15점) - 가중치 감소
    const dayJiScore = this.calculateDayJiCompatibility(person1, person2);
    score += dayJiScore * 0.15;

    // 4. 애정살 (10점) - 가중치 감소
    const loveStarScore = this.calculateLoveStarCompatibility(person1, person2);
    score += loveStarScore * 0.1;

    // 5. 체질 상성 제거 - 너무 높은 점수 방지
  ...
  calculatePersonalityCompatibility(person1, person2) {
    ...
    const intimacy = this.calculateModernIntimacyCompatibility(
      person1,
      person2
    );
  ...
  generateModernAnalysis(person1, person2, overallScore, intimacyScore) {
    ...
    if (overallScore >= 80) {
      ...
      analysis.push(
        "💪 서로의 차이를 인정하고 노력하면 좋은 관계가 될 수 있습니다."
      );
    }
